...
def rag_query():
    try:
        ...
        # Try Qwen LLM first
        response = get_qwen_response(question)
        
        print(f"Generated response: {response[:100]}...")
        
        return jsonify({
            'answer': response,
            'sources': [{'title': 'Qwen2.5-1.5B Medical AI', 'relevance_score': 0.95}],
            'llm_used': 'Qwen2.5-1.5B-Instruct',
...
def predict_with_rag():
    ...
    prediction_result['rag_info'] = {
        'summary': 'Brain tumor classification completed using Qwen2.5-1.5B AI for medical information.',
        'sources': [{'title': 'Qwen2.5-1.5B Medical AI Assistant'}]
    }
