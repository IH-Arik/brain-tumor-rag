...
def setup_langchain_rag():
    ...
    try:
        ...
        try:
            ...
            print("⚠️ Hugging Face Hub failed, using fallback")
            llm = None
...
def get_langchain_response(question):
    """Get response from LangChain RAG system"""
    try:
        if rag_chain:
            print("🔗 Using LangChain RAG...")
...
def rag_query():
    try:
        ...
        response, sources = get_langchain_response(question)
        ...
        return jsonify({
            'answer': response,
            'sources': source_info,
            'llm_used': 'LangChain RAG' if rag_chain else 'Enhanced Keyword System',
...
def predict_with_rag():
    ...
    prediction_result['rag_info'] = {
        'summary': 'Brain tumor classification completed using LangChain RAG system for enhanced medical information.',
        'sources': [{'title': 'LangChain RAG Knowledge Base'}]
    }
